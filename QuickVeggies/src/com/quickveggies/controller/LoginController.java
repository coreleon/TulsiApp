        ...
        if(dbclient.getRowsNum("users")>0)
        {
            ...
            register.setOnAction(new EventHandler<ActionEvent>()
            {
                public void handle(ActionEvent event)
                {
                            ...
                            try
                            {
                                ...
                                if(input.equals(realPwd))
                                {
                                    ...
                                    register.setOnAction(new EventHandler<ActionEvent>()
                                    {
                                        public void handle(ActionEvent event)
                                        {
                                            new Main().replaceSceneContent("/register.fxml");
                                        }
                                    });
                                }
                                else
                                {
                                    GeneralMethods.errorMsg("incorrect password!");
                                }
                    ...
                    CustomDialogController controller = new CustomDialogController(new Button[]
                    {
                        ok, cancel
                    }, pwdField, "Retype login password:");

                    GeneralMethods.openNewWindow(LoginController.this, "/customdialog.fxml", controller, null);
        ...
        loginButton.setOnAction(new EventHandler<ActionEvent>()
        {
            public void handle(ActionEvent event)
            {
                if(!username.getText().equals("")&&!pass.getText().equals(""))
                {
                    ...
                    if(UserGlobalParameters.SQLURL.equals(""))
                    {
                        UserGlobalParameters.SQLURL = "localhost";
                    }
                    if(UserGlobalParameters.SQLUSER.equals(""))
                    {
                        UserGlobalParameters.SQLUSER = "qvuser";
                    }
                    if(UserGlobalParameters.SQLPASS.equals(""))
                    {
                        UserGlobalParameters.SQLPASS = "qvdbusr123";
                    }

                    DatabaseClient dbclient = DatabaseClient.getInstance();
                    if(dbclient==null)
                    {
                        GeneralMethods.errorMsg("Wrong SQL server address/login parameters");
                        return;
                    }
...
//                            	       if(SessionDataController.getCurrentUser().getUnsavedWindows()>0)
...
//                            	});
                ...
                {
                    GeneralMethods.errorMsg("Not all fields were properly filled!");
                }
